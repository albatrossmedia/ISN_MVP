/*
  ...
  **Columns:**
  ...
  - `type` (text, not null) - Type of dataset: 'audio', 'text', 'video', 'mixed'
  - `size_gb` (numeric) - Size of the dataset in gigabytes
  - `samples` (integer) - Number of samples in the dataset
  - `languages` (text[]) - Array of language codes supported
  - `version` (text) - Current version of the dataset
  - `quality_score` (numeric) - Quality score from 0-100
  - `license` (text) - License type (MIT, Apache-2.0, etc.)
  - `status` (text, not null) - Status: 'active', 'deprecated', 'processing'
  - `downloadable` (boolean, default true) - Whether dataset can be downloaded
  ...
  **Columns:**
  ...
  - `type` (text, not null) - Type: 'ASR', 'MT', 'QA', 'ContextNet', 'Lexicon'
  - `version` (text) - Current version of the model
  - `status` (text, not null) - Status: 'active', 'training', 'deprecated'
  - `accuracy` (numeric) - Model accuracy percentage
  - `languages` (text[]) - Array of supported language codes
  - `parameters` (text) - Model parameter information
  - `framework` (text) - ML framework used (TensorFlow, PyTorch, etc.)
  - `created_at` (timestamptz, default now()) - Creation timestamp
  - `updated_at` (timestamptz, default now()) - Last update timestamp
  ...
  **Columns:**
  ...
  - `created_at` (timestamptz, default now()) - When the relationship was created
